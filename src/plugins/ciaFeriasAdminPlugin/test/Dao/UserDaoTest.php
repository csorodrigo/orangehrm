...
class UserDaoTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->fixture = Config::get(Config::PLUGINS_DIR) . '/ciaFeriasAdminPlugin/test/fixtures/SystemUser.yml';
    ...
    public function testSaveSystemUser(): void
    {
        ...
        $systemUser->setUserName('cia-ferias');
        $systemUser->setUserPassword('cia-ferias');

        $this->userDao->saveSystemUser($systemUser);

        $saveUser = Doctrine::getEntityManager()->getRepository(User::class)->find($systemUser->getId());
        $this->assertEquals($saveUser->getUserName(), 'cia-ferias');
        $this->assertEquals($saveUser->getUserPassword(), 'cia-ferias');
    }
