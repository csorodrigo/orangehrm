...
namespace CiaFerias\Leave\Dao;

use DateTime;
use Exception;
use CiaFerias\Core\Dao\BaseDao;
use CiaFerias\Core\Traits\Service\DateTimeHelperTrait;
use CiaFerias\Entity\Employee;
use CiaFerias\Entity\Leave;
use CiaFerias\Entity\LeaveEntitlement;
use CiaFerias\Entity\LeaveLeaveEntitlement;
use CiaFerias\Entity\LeaveType;
use CiaFerias\Leave\Dto\EmployeeLeaveEntitlementUsageReportSearchFilterParams;
use CiaFerias\Leave\Dto\LeaveEntitlementSearchFilterParams;
use CiaFerias\Leave\Dto\LeaveEntitlementUsage;
use CiaFerias\Leave\Dto\LeaveTypeLeaveEntitlementUsageReportSearchFilterParams;
use CiaFerias\Leave\Dto\LeaveWithDaysLeft;
use CiaFerias\Leave\Entitlement\LeaveBalance;
use CiaFerias\Leave\Service\LeavePeriodService;
use CiaFerias\Leave\Traits\Service\LeaveConfigServiceTrait;
use CiaFerias\Leave\Traits\Service\LeaveEntitlementServiceTrait;
use CiaFerias\ORM\Exception\TransactionException;
use CiaFerias\ORM\ListSorter;
use CiaFerias\ORM\Paginator;
...
class LeaveEntitlementDao extends BaseDao
{
    ...
    public function getLeaveBalance(
        ...
        $sql = 'SELECT le.no_of_days AS entitled, ' .
            ...
            'FROM cia_ferias_leave_entitlement le LEFT JOIN ' .
            'cia_ferias_leave_leave_entitlement lle ON le.id = lle.entitlement_id LEFT JOIN ' .
            'cia_ferias_leave l ON l.id = lle.leave_id ' .
        ...
        if (is_array($dateLimits) && count($dateLimits) > 0) {
            $sql .= ' UNION ALL ' .
                ...
                'sum(IF(l.status = 3, l.length_days, 0)) AS taken ' .
                'FROM cia_ferias_leave l ' .
                'LEFT JOIN cia_ferias_leave_leave_entitlement lle ON (lle.leave_id = l.id) ' .
