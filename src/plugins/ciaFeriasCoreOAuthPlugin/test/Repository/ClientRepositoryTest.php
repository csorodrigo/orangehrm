...
use CiaFerias\OAuth\Repository\ClientRepository;
...
class ClientRepositoryTest extends KernelTestCase
{
    protected function setUp(): void
    {
        $fixture = Config::get(Config::PLUGINS_DIR)
            . '/ciaFeriasCoreOAuthPlugin/test/fixtures/ClientRepositoryTest.yaml';
    ...
    public function testGetClientEntity(): void
    {
        ...
        $clientEntity = $clientRepository->getClientEntity('cia_ferias_mobile_app');
        $this->assertEquals(1, $clientEntity->getIdentifier());
        $this->assertEquals('cia_ferias_mobile_app', $clientEntity->getName());
        $this->assertEquals('com.ciaferias.opensource://oauthredirect', $clientEntity->getRedirectUri());
        $this->assertFalse($clientEntity->isConfidential());
        $this->assertEquals('CIA Férias Mobile App', $clientEntity->getDisplayName());
    ...
    public function testValidateClient(): void
    {
        ...
        $valid = $clientRepository->validateClient('cia_ferias_mobile_app', '', 'refresh_token');
        $this->assertTrue($valid); // non-confidential client with `refresh_token` is valid everytime

        $valid = $clientRepository->validateClient('cia_ferias_mobile_app', '', 'authorization_code');
