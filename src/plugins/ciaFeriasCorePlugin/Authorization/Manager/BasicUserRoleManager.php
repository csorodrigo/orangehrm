...
namespace CiaFerias\Core\Authorization\Manager;

use CiaFerias\Admin\Traits\Service\UserServiceTrait;
use CiaFerias\Core\Authorization\Dao\HomePageDao;
...
use CiaFerias\Core\Authorization\Exception\AuthorizationException;
use CiaFerias\Core\Authorization\Service\DataGroupService;
use CiaFerias\Core\Authorization\Service\ScreenPermissionService;
use CiaFerias\Core\Authorization\UserRole\AbstractUserRole;
use CiaFerias\Core\Exception\CoreServiceException;
use CiaFerias\Core\HomePage\HomePageEnablerInterface;
use CiaFerias\Core\Service\AccessFlowStateMachineService;
use CiaFerias\Core\Service\MenuService;
use CiaFerias\Core\Traits\ClassHelperTrait;
...
class BasicUserRoleManager extends AbstractUserRoleManager
{
    ...
    private function _init(): void
    {
        ...
        foreach ($configurations as $roleName => $roleObj) {
            $className = $this->getClassHelper()->getClass(
                $roleObj['class'],
                'CiaFerias\\Core\\Authorization\\UserRole\\'
            );
    ...
    public function getModuleDefaultPage(string $module): ?string
    {
        ...
        foreach ($defaultPages as $defaultPage) {
            ...
            $fallbackNamespace = 'CiaFerias\\Core\\HomePage\\';
     ...
     * @return string
     * @todo fix this in `cia_ferias_workflow_state_machine`
     */
