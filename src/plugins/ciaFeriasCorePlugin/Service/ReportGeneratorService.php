...
namespace CiaFerias\Core\Service;

use CiaFerias\Core\Api\V2\ParameterBag;
use CiaFerias\Core\Dao\ReportGeneratorDao;
use CiaFerias\Core\Dto\FilterParams;
use CiaFerias\Core\Report\DisplayField\BasicDisplayField;
use CiaFerias\Core\Report\DisplayField\CombinedDisplayField;
use CiaFerias\Core\Report\DisplayField\EntityAliasMapping;
use CiaFerias\Core\Report\DisplayField\GenericBasicDisplayField;
use CiaFerias\Core\Report\DisplayField\GenericDateDisplayField;
use CiaFerias\Core\Report\DisplayField\ListableDisplayField;
use CiaFerias\Core\Report\DisplayField\NormalizableDTO;
use CiaFerias\Core\Report\DisplayField\Stringable;
use CiaFerias\Core\Report\FilterField\FilterField;
use CiaFerias\Core\Report\FilterField\ValueXModifiable;
use CiaFerias\Core\Report\FilterField\ValueYModifiable;
use CiaFerias\Core\Report\Header\Column;
use CiaFerias\Core\Report\Header\Header;
use CiaFerias\Core\Report\Header\HeaderData;
use CiaFerias\Core\Report\Header\StackedColumn;
use CiaFerias\Core\Report\ReportSearchFilterParams;
use CiaFerias\Core\Traits\ORM\EntityManagerHelperTrait;
use CiaFerias\Entity\AbstractDisplayField;
use CiaFerias\Entity\CompositeDisplayField;
use CiaFerias\Entity\DisplayField;
use CiaFerias\Entity\Employee;
use CiaFerias\Entity\Report;
use CiaFerias\Entity\SelectedFilterField;
use CiaFerias\Entity\SummaryDisplayField;
use CiaFerias\I18N\Traits\Service\I18NHelperTrait;
use CiaFerias\ORM\Exception\TransactionException;
use CiaFerias\ORM\QueryBuilderWrapper;
...
class ReportGeneratorService
{
    ...
    protected function getReportDataQueryBuilder(ReportSearchFilterParams $filterParams): array
    {
        ...
        foreach ($displayFields as $displayField) {
            ...
            if ($displayFieldClass instanceof \CiaFerias\Core\Report\DisplayField\DisplayField) {
    ...
    public function savePimDefinedReport(
        ...
        foreach ($fieldGroup as $key => $value) {
            // creating an array that contains the display field group id which selected as header by user(`cia_ferias_display_field_group` table)
            if ($value['includeHeader'] ?? false) {
                array_push($selectedDisplayFieldGroupIds, $key);
            }

            foreach ($value['fields'] as $field) {
                /** creating an array that contains the display field id (`cia_ferias_display_field` table)
                 * Here fields is the value pair of $fieldGroup associative array which  contains the id of `cia_ferias_display_field` table
                 **/
