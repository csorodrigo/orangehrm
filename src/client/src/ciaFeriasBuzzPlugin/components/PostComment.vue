...
<template>
  <div class="cia-ferias-comment-wrapper">
    <profile-image :employee="data.employee"></profile-image>
    <div class="cia-ferias-post-comment">
      <oxd-form v-if="edit" @submit-valid="onSubmit">
        ...
        <oxd-text
          tag="span"
          class="cia-ferias-post-comment-action --cancel"
          @click="onCancelComment"
        >
          {{ $t('general.cancel') }}
        </oxd-text>
      </oxd-form>
      <div v-else class="cia-ferias-post-comment-area">
        <oxd-text tag="p" class="cia-ferias-post-comment-employee">
          {{ employeeFullName }}
        </oxd-text>
        <oxd-text
          tag="span"
          :class="{
            'cia-ferias-post-comment-text': true,
        ...
        <oxd-text
          v-show="!readMore"
          tag="span"
          class="cia-ferias-post-comment-readmore"
          @click="onClickReadMore"
        >
          {{ $t('buzz.read_more') }}
        </oxd-text>
        <oxd-text tag="p" class="cia-ferias-post-comment-datetime">
          {{ dateTime }}
        </oxd-text>
        <div
          v-if="data.comment.numOfLikes > 0"
          class="cia-ferias-post-comment-stats"
        >
          <oxd-icon
            name="heart-fill"
            class="cia-ferias-post-comment-stats-icon"
          />
          <oxd-text tag="p" class="cia-ferias-post-comment-stats-text">
            {{ data.comment.numOfLikes }}
          </oxd-text>
        </div>
      </div>
      <div v-if="!edit" class="cia-ferias-post-comment-action-area">
        <oxd-text
          tag="p"
          :class="{
            'cia-ferias-post-comment-action': true,
        ...
        <oxd-text
          ...
          tag="p"
          class="cia-ferias-post-comment-action"
          @click="onClickEdit"
        >
          {{ $t('general.edit') }}
        </oxd-text>
