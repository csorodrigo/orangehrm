...
<template>
  <div class="cia-ferias-buzz-post-body">
    ...
    <oxd-text
      v-show="!readMore"
      tag="p"
      class="cia-ferias-buzz-post-body-readmore"
    ...
    <photo-frame v-if="post.type === 'photo'" :media="post.photoIds">
      <template #content="{index}">
        <div
          ...
          class="cia-ferias-buzz-post-body-picture --more"
          @click="onClickPicture(index)"
        >
          <oxd-text tag="p" class="cia-ferias-buzz-post-body-more">+1</oxd-text>
          <oxd-icon class="cia-ferias-buzz-post-body-more" name="images" />
        </div>
        <div
          v-else
          class="cia-ferias-buzz-post-body-picture"
          @click="onClickPicture(index)"
        ></div>
      </template>
    </photo-frame>
    <template v-if="originalPost">
      <br v-if="post.text || post.type === 'video' || post.type === 'photo'" />
      <oxd-text tag="p" class="cia-ferias-buzz-post-body-employee">
        {{ originalPost.employee }}
      </oxd-text>
      <oxd-text tag="p" class="cia-ferias-buzz-post-body-date">
        {{ originalPost.dateTime }}
      </oxd-text>
      <oxd-text
        v-if="originalPost.text"
        tag="p"
        class="cia-ferias-buzz-post-body-original-text"
      >
        {{ originalPost.text }}
      </oxd-text>
    </template>
...
import PhotoFrame from '@/ciaFeriasBuzzPlugin/components/PhotoFrame';
...
export default {
  ...
  setup(props, context) {
    ...
    const postClasses = computed(() => ({
      'cia-ferias-buzz-post-body-text': true,
