...
<template>
  <oxd-sheet
    ...
    :gutters="false"
    type="white"
    class="cia-ferias-buzz-stats-modal"
  >
    <div
      v-for="user in users"
      :key="user"
      class="cia-ferias-buzz-stats-modal-employee"
    >
      <profile-image :employee="user.employee"></profile-image>
      <oxd-text tag="p" class="cia-ferias-buzz-stats-modal-employee-name">
        {{ user.fullName }}
      </oxd-text>
    </div>
    <oxd-loading-spinner v-if="isLoading" class="cia-ferias-buzz-loader" />
  </oxd-sheet>
  <oxd-dialog
    v-else
    class="cia-ferias-buzz-stats-dialog"
    @update:show="onClose"
  >
    <div class="cia-ferias-buzz-stats-dialog-header">
      <oxd-icon
        :class="{
          'cia-ferias-buzz-stats-dialog-icon': true,
          '--likes': type === 'likes',
        }"
        :name="icon"
        :with-container="true"
    ...
    <div
      v-for="user in users"
      :key="user"
      class="cia-ferias-buzz-stats-dialog-employee"
    >
      <profile-image :employee="user.employee"></profile-image>
      <oxd-text tag="p" class="cia-ferias-buzz-stats-dialog-employee-name">
        {{ user.fullName }}
      </oxd-text>
    </div>
    <oxd-loading-spinner v-if="isLoading" class="cia-ferias-buzz-loader" />
...
import {OxdDialog, OxdIcon, OxdSheet, OxdSpinner} from '@cia-ferias/oxd';
