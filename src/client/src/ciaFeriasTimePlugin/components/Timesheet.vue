...
<template>
  <oxd-form class="cia-ferias-paper-container">
    <div class="cia-ferias-timesheet-header">
      <div class="cia-ferias-timesheet-header--title">
        <slot name="header-title"></slot>
      </div>
      <div class="cia-ferias-timesheet-header--options">
        <slot name="header-options"></slot>
      </div>
    </div>

    <div v-if="loading" class="cia-ferias-timesheet-loader">
      <oxd-loading-spinner />
    </div>
    <div
      v-else-if="!loading && !columns"
      class="cia-ferias-timesheet-body-message"
    >
      <oxd-alert
        type="warn"
        :show="true"
    ...
    <div v-else class="cia-ferias-timesheet-body">
      <table :class="tableClasses">
        <thead class="cia-ferias-timesheet-table-header">
          <tr class="cia-ferias-timesheet-table-header-row">
            <th :class="fixedColumnClasses">
              {{ $t('time.project') }}
            </th>
            <th class="cia-ferias-timesheet-table-header-cell">
              {{ $t('time.activity') }}
            </th>
            ...
            <th
              ...
              :key="day.id"
              class="cia-ferias-timesheet-table-header-cell --center"
            >
              <span class="--day">
                {{ day.day }}
              </span>
            ...
            <th
              v-if="!editable"
              class="cia-ferias-timesheet-table-header-cell --center --freeze-right"
            >
              {{ $t('general.total') }}
            </th>
          </tr>
        ...
        <tbody class="cia-ferias-timesheet-table-body">
          <!-- timesheet activities -->
          <tr
            v-for="(record, i) in records"
            :key="record"
            class="cia-ferias-timesheet-table-body-row"
          >
            ...
            <td class="cia-ferias-timesheet-table-body-cell">
              <activity-dropdown
                v-if="editable"
            ...
            <td
              ...
              :class="{
                'cia-ferias-timesheet-table-body-cell': true,
                '--center': true,
            ...
            >
              <oxd-icon-button
                ...
                class="cia-ferias-timesheet-icon-comment"
            ...
            <td
              v-if="!editable"
              class="cia-ferias-timesheet-table-body-cell --center --freeze-right --highlight"
            >
              {{ record.total.label }}
            </td>
            <td
              v-if="editable"
              class="cia-ferias-timesheet-table-body-cell --flex"
            >
              <oxd-icon-button
                name="trash"
                class="cia-ferias-timesheet-icon"
                @click="deleteRow(i)"
              />
            </td>
          </tr>
          ...
          <tr
            ...
            class="cia-ferias-timesheet-table-body-row --total"
          >
            <td
              class="cia-ferias-timesheet-table-body-cell --freeze-left --highlight"
            >
              {{ $t('general.total') }}
            </td>
            <td></td>
            ...
            <td
              ...
              :key="`total-${date}`"
              class="cia-ferias-timesheet-table-body-cell --center"
            >
              {{ date.total.label }}
            </td>
            <!-- total per day -->
            <td
              class="cia-ferias-timesheet-table-body-cell --center --freeze-right --highlight-2"
            >
              {{ subtotal }}
            </td>
          </tr>
          ...
          <!-- add row -->
          <tr v-if="editable" class="cia-ferias-timesheet-table-body-row">
            <td class="cia-ferias-timesheet-table-body-cell --flex">
              <oxd-icon-button
                name="plus"
                class="cia-ferias-timesheet-icon"
                @click="addRow"
          ...
          <tr
            ...
            class="cia-ferias-timesheet-table-body-row"
          >
            <td colspan="9" class="cia-ferias-timesheet-table-body-cell">
    ...
    <div class="cia-ferias-timesheet-footer">
      <div class="cia-ferias-timesheet-footer--title">
        <slot name="footer-title"></slot>
      </div>
      <div class="cia-ferias-timesheet-footer--options">
...
import ActivityDropdown from '@/ciaFeriasTimePlugin/components/ActivityDropdown.vue';
import ProjectAutocomplete from '@/ciaFeriasTimePlugin/components/ProjectAutocomplete.vue';
import TimesheetCommentModal from '@/ciaFeriasTimePlugin/components/TimesheetCommentModal.vue';
import {OxdAlert, OxdSpinner} from '@cia-ferias/oxd';
...
export default {
  ...
  computed: {
    ...
    tableClasses() {
      return {
        'cia-ferias-timesheet-table': true,
        '--editable': this.editable,
      };
    },
    fixedColumnClasses() {
      return {
        'cia-ferias-timesheet-table-header-cell': true,
