...
<template>
  <div class="cia-ferias-timeperiod-picker">
    <oxd-text tag="p" class="cia-ferias-timeperiod-title">
      {{ $t('time.timesheet_period') }}
    </oxd-text>
    <oxd-icon-button
      class="cia-ferias-timeperiod-icon --prev"
      name="chevron-left"
      @click="$emit('previous', $event)"
    />
    <date-input v-bind="$attrs" />
    <oxd-icon-button
      class="cia-ferias-timeperiod-icon --next"
      name="chevron-right"
