      ...
      <div v-else class="ca-vacation-planning__grid">
        ...
        >
                ...
                >
                  ...
                  >
                    <label>
                      ...
                      <input
                        v-model="
                          preferenceForm[preferenceFormKey(option.label)]
                            .fromDate
                        "
                        type="date"
                      />
                    ...
                    <label>
                      ...
                      <input
                        v-model="
                          preferenceForm[preferenceFormKey(option.label)].toDate
                        "
                        type="date"
                      />
                    </label>
          ...
          <div class="ca-vacation-card__reasons">
            <span
              v-for="reason in visibleReasons(plan).slice(0, 3)"
              :key="reason"
            >
              {{ reason }}
            </span>
      ...
      <aside class="ca-vacation-drawer" role="dialog" aria-modal="true">
        ...
        <div class="ca-vacation-drawer__section">
          ...
          <div class="ca-vacation-drawer__reasons">
            <span v-for="reason in drawerReasons(selectedPlan)" :key="reason">
              {{ reason }}
            </span>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    isSelected(plan) {
      return this.selectedPlan?.employee?.empNumber === plan.employee.empNumber;
    },
    recommendationStart(plan) {
        ...
        return {
          ...
          period:
            option?.fromDate && option?.toDate
              ? `${this.formatDate(option.fromDate)} ate ${this.formatDate(
                  option.toDate,
                )}`
    ...
    acquisitionNotice(plan) {
      return plan.reasons?.find((reason) => reason.includes('12 meses')) || '';
    },
    visibleReasons(plan) {
    ...
    async generateVacationPlanningPdf() {
      ...
      const reportStyle =
        documentHtml.querySelector('style')?.textContent || '';
    ...
    createVacationPlanningReportHtml() {
      ...
      const rows = this.plans
        .map((plan) => this.createReportRow(plan))
        .join('');
  ...
  ${scriptClose}
    ...
    createReportRow(plan) {
      ...
      return `<div class="report-table__row">
        ...
        <span>${this.escapeHtml(
          plan.employee.subunit || 'Setor nao informado',
        )}</span>
        ...
        <span><mark class="${this.reportPreferenceClass(
          preference,
        )}">${this.escapeHtml(preference)}</mark></span>
      </div>`;
    },
    createReportCss() {
      return `
...
.ca-vacation-table__row {
  ...
  grid-template-columns:
    minmax(7.5rem, 1.35fr) minmax(0, 1fr) minmax(0, 0.9fr) minmax(7.5rem, 1.2fr)
    minmax(4.8rem, 0.7fr);
  gap: 0;
  align-items: center;
}
